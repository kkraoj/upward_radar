...
function [incoherentSumReturned,numChirpsDetected,noChirpsFiles] = coherentIncoherentSums_ver8_modForC(fs,directory,windowSize,dataType,numPhaseShifts,phaseRange,display)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%READ IN Match Filter
fID = fopen(directory); %open data file
R = (fread(fID,'float')); %read data  ,'ieee-be'?
fclose(fID);
lags = R(1:2:length(R));
R=single(R(2:2:length(R))); %only abs is spit out
...
if isempty(locs)
    ...
    while (n<=length(locs))
        ...
        %start of the chirp
        if ((locs(n)< (sampsPerChirp+1))) %edge must be farther than sampsPerChirp for summation algorithm
            %remove peak from list if chirp is clipped
            locs(n)=[];
            peaks(n)=[];
        elseif ((locs(n)> (length(R)-sampsPerChirp-1)))
            %remove peak from list if chirp is clipped
            locs(n)=[];
            peaks(n)=[];
        else
    ...
    %% zoom in on each rx chirp and find best phase shift
        incoherentSumReturned =zeros(windowSize+1,1);

    for n=1:numChirpsDetected
        %grab time domain data centered around chirp at 2x chirp width
        dataCenter=directPathLocs(n); %sampsPerChirp is always even
        d1 = dataCenter - windowSize/2;
        d2 = dataCenter + windowSize/2;
        %data_sub=R(d1:d2);
...
%     end
    ...
    %create incoherent sums
        incoherentSumReturned=incoherentSumReturned+R(d1:d2);
end
