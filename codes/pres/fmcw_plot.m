        ...
        elseif vdat.Code == -5
            ...
            if plot_raw
                ...
                for ci = 1:length(vdat.Startind)
                    ...
                    h = plot(sn(vdat.Startind(ci):vdat.Endind(ci)),vdat.v(vdat.Startind(ci):vdat.Endind(ci)),'LineWidth',3,'userData','fmcw');
            ...
            if ~isempty(BurstChirpList)
                ...
                for AttSetNum = 1:length(vdats) % attenuator setting number
                    ...
                    if plot_time
                        ...
                        axes(tax), hold on
                        for ci = 1:size(vdat.vif,1)
                            h = plot(vdat.t,vdat.vif(ci,:),'tag',chirpname{ci},'color',color,'LineWidth',3,'userData','fmcw'); % signal
                            setappdata(h,'colorMode',colorMode)
                    ...
                    if plot_amp_phase
                        ...
                        axes(pax), hold on
                        for ci = 1:size(vdat.vif,1)
                            h = plot(rc,angle(su(ci,:)),'tag',chirpname{ci},'color',color,'LineWidth',3,'userData','fmcw');
                            setappdata(h,'colorMode',colorMode)
                            set(findall(gcf,'-property','FontSize'),'FontSize',20)
                        end
                        axes(aax), hold on
                        for ci = 1:size(vdat.vif,1)
                            h = plot(rc,20*log10(abs(su(ci,:))),'tag',chirpname{ci},'color',color,'LineWidth',3,'userData','fmcw');
                            setappdata(h,'colorMode',colorMode)
                            set(findall(gcf,'-property','FontSize'),'FontSize',20)
                        end
                    end
...
if ishandle(ax)
    ...
    if ~isempty(h)
        ...
        if n>1
            for ii = 1:n
                set(h(ii),'col',getcol([0 n+1],ii,flipud(parula)))
            end
        end
    end
end
